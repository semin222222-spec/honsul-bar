  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        ctx.drawImage(
          img,
          sx, sy, minSide, minSide,    // 소스 (정사각형 영역)
          0, 0, TARGET_SIZE, TARGET_SIZE // 대상
        );
...
export async function uploadMenuImage(file, storeId, onProgress) {
  try {
    ...
    return { ok: true, url: urlData.publicUrl, path: filePath };
  } catch (err) {
...
export async function deleteMenuImage(imageUrl) {
  try {
    ...
    // URL에서 path 추출
    // 예: https://xxx.supabase.co/storage/v1/object/public/menu-images/store-id/123-abc.jpg
