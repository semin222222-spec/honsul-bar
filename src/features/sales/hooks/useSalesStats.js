...
export function useSalesStats(storeId) {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      [
        ...
        salesRepository.listOrdersSince({
          storeId,
          since: todayStart.toISOString(),
        }),
        salesRepository.listOrdersBetween({
          storeId,
        ...
        salesRepository.listOrdersForMonthlyHistory({
          storeId,
          since: sixMonthsAgo.toISOString(),
        }),
        salesRepository.listSessionsSince({
          storeId,
          since: sixMonthsAgo.toISOString(),
          columns: "opened_at",
        }),
    ...
    const yesterdayRevenue = yesterdayList.reduce(
      (sum, o) => sum + (o.price || 0),
      0,
    );
    ...
    const hourlyRevenue = HOURS.map((h) => ({
      hour: h,
      label:
        h === 0 ? "12am" : h === 12 ? "12pm" : h < 12 ? `${h}` : `${h - 12}`,
    ...
    const avgPerCustomer =
      sessions.length > 0 ? Math.round(todayRevenue / sessions.length) : 0;
  ...
  useEffect(() => {
    const fetchTimer = setTimeout(fetchStats, 0);

    if (!storeId) return () => clearTimeout(fetchTimer);

    // 새 주문 생기면 통계 재계산
    const unsubscribe = salesRepository.subscribeToSalesStats({
      storeId,
      onChange: () => fetchStats(),
      onStatus: (status) => {
        handleRealtimeSubscribeStatus(status, {
          label: "Sales Stats",
          onSubscribed: fetchStats,
          onRecoverable: fetchStats,
        });
      },
    });
