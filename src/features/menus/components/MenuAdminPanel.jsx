...
import {
  autoTranslateMenu,
  translateText,
} from "@/shared/lib/translateService";
import { uploadMenuImage, deleteMenuImage } from "@/shared/lib/imageUpload";
// 🚧 재고 관리 시스템 - 영업 안정화 후 활성화 예정
// import { useMenuRecipes } from "@/features/menus/hooks/useMenuRecipes";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div
            style={{ fontSize: 11, color, fontWeight: 600, marginBottom: 4 }}
          >
            ● {name || "(이름 없음)"}
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            onClick={onClose}
            style={{ ...btnStyle, ...cancelBtnStyle, flex: 1 }}
          >
            취소
          </button>
          <button
...
// 🚧 레시피 관련 컴포넌트들은 재고 관리 시스템 완성 후 활성화 예정
// (RecipeRow, AddRecipeForm, RecipeSection은 useMenuRecipes/useInventory가 필요해서 임시 제거)
...
function MenuModal({
  ...
  const [categoryId, setCategoryId] = useState(
    menu?.category_id || categories[0]?.id || "",
  );
  ...
  const currentCategoryGroups = (existingGroups[categoryId] || []).filter(
    (g) => g && g.trim(),
  );
  ...
  const handleImageUpload = async (e) => {
    ...
    setUploadProgress(0);
    if (result.ok) {
      if (imageUrl) deleteMenuImage(imageUrl, storeId);
  ...
  const handleSave = async () => {
    ...
    if (!hasOptions && !price) {
      return alert(
        "가격을 입력해주세요 (옵션을 추가하면 가격은 옵션에서 관리됩니다)",
      );
    }
    setSaving(true);
          ...
          >
            ...
            {categories.map((cat) => (
              <option key={cat.id} value={cat.id}>
                {cat.name}{" "}
                {cat.default_price
                  ? `(${cat.default_price.toLocaleString()}원)`
                  : ""}
              </option>
            ))}
          </select>
                ...
                {currentCategoryGroups.map((g) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 100,
                      color:
                        groupName === g ? "#D4A537" : "rgba(255,255,255,0.5)",
                      fontSize: 10,
                      cursor: "pointer",
                      fontFamily: "inherit",
                    }}
        ...
        {menu && (
          <div style={{ marginBottom: 12 }}>
            ...
            {hasOptions && (
              ...
              >
                💡 옵션이 있으면 손님은 옵션을 선택해 주문합니다. 아래 "가격"은
                무시됩니다.
              </div>
            )}
          </div>
        ...
        >
          <div>
            <label style={labelStyle}>
              가격 (원)
              {hasOptions && (
                <span style={{ color: "rgba(255,180,180,0.7)", marginLeft: 4 }}>
                  (옵션 사용중)
                </span>
              )}
            </label>
            <input
              ...
              onChange={(e) => setPrice(e.target.value)}
              placeholder="9900"
              style={{ ...inputStyle, opacity: hasOptions ? 0.4 : 1 }}
              disabled={hasOptions}
            />
          </div>
          <div>
        ...
        >
          <div>
            <div style={{ fontSize: 12, color: "#F5E6C8", marginBottom: 2 }}>
              메뉴 활성화{" "}
              {!isActive && (
                <span style={{ color: "rgba(255,180,180,0.85)" }}>(품절)</span>
              )}
            </div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>
    ...
    <div ref={setNodeRef} style={style}>
      <div
        style={{
          ...
          border:
            "1px solid " +
            (isDragging ? "rgba(212,165,55,0.4)" : "rgba(255,255,255,0.04)"),
          borderRadius: 10,
...
export default function MenuAdminPanel({
  ...
  const [, setIsDraggingMenu] = useState(false);
  ...
  const handleDragEnd = async (event, categoryId) => {
    ...
    try {
      ...
      if (groupChanged) {
        showToast(
          `✓ "${movedMenu.name}" → ${newGroupName || "그룹 없음"}로 이동`,
        );
      } else {
        showToast(`✓ 순서 변경됨`);
      }
  ...
  return (
    <div>
      ...
      >
        ...
        />{" "}
        핸들을 길게 누른 후 원하는 위치로 끌어 순서를 바꿀 수 있어요
      </div>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                <button
                  ...
                  style={{ ...iconBtnStyle, opacity: isFirst ? 0.2 : 1 }}
                  title="위로"
                >
                  <ChevronUp size={14} />
                </button>
                <button
                  ...
                  style={{ ...iconBtnStyle, opacity: isLast ? 0.2 : 1 }}
                  title="아래로"
                >
                  <ChevronDown size={14} />
                </button>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {orphanMenus.map((menu) => (
              ...
              >
                ...
                <div style={{ flex: 1, fontSize: 12, color: "#F5E6C8" }}>
                  {menu.name}
                </div>
        ...
        {(editingCategory || showNewCategory) && (
          <CategoryModal
            ...
            onSave={async (data) => {
              ...
              if (result.ok) {
                showToast(
                  editingCategory ? "✓ 카테고리 수정됨" : "✓ 카테고리 추가됨",
                );
                setEditingCategory(null);
                setShowNewCategory(false);
              }
