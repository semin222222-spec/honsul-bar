...
function SeatRowModal({ row, existingNames, onClose, onSave, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    const duplicate = existingNames.some(
      (n) => n.name === trimmedName && n.id !== row?.id,
    );
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          >
            영문/숫자/한글 가능. 좌석 이름은 "{name || "?"}-1, {name || "?"}
            -2..." 형태가 됩니다
          </div>
        </div>
        ...
        {row && count < row.seat_count && (
          ...
          >
            ⚠ {row.seat_count}석 → {count}석으로 줄어들어요.
            <br />
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            {rows.length}개 행 · 총 {totalSeats}석 · 현재 {totalOccupied}명
            이용중
          </div>
        </div>
        <button
              ...
              >
                ...
                <div
                  style={{
                    ...
                    borderRadius: 12,
                    background:
                      "linear-gradient(135deg, rgba(212,165,55,0.15), rgba(180,120,30,0.05))",
      ...
      >
        💡 <strong>안내</strong>
        <br />
        · 행 이름이 좌석 라벨이 됩니다 (예: "A" → A-1, A-2)
        <br />
        · 좌석을 줄이기 전에 손님 자리를 먼저 옮겨주세요
        <br />· 행을 삭제해도 기존 주문/매출 기록은 유지됩니다
      </div>
