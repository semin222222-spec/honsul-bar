...
import { useSeatOccupancy } from "@/features/seats/hooks/useSeatOccupancy";
...
export default function SeatPicker({ onSelect }) {
  ...
  const { occupiedSeats } = useSeatOccupancy(storeId);
        ...
        {rowsLoading ? (
          ...
          rows.map((row, rowIdx) => {
            ...
            return (
              <div
                key={row.id}
                style={{ marginBottom: rowIdx === rows.length - 1 ? 24 : 16 }}
              >
                <div
                  style={{
                    display: "flex",
