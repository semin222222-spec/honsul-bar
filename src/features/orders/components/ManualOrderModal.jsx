...
import { createManualOrder } from "@/services/orders/orderService";
...
export default function ManualOrderModal({
  ...
  const currentOptions = useMemo(
    () => (selectedMenu ? optionsByMenu.get(selectedMenu.id) || [] : []),
    [selectedMenu, optionsByMenu],
  );
  ...
  const handleSubmit = async () => {
    const scopedStoreId = storeId || session?.store_id;
    if (!canSubmit || !session || !hasStoreScope(scopedStoreId)) return;
    setSubmitting(true);
    ...
    try {
      ...
      onSuccess?.(
        `${session.seat_label}에 "${selectedMenu.name}${optionLabel}"${qtyLabel} 추가됨`,
      );
      onClose();
    } catch (err) {
    ...
    return (
      <Motion.div
        ...
        style={{
          ...
          border:
            "1px solid " +
            (isSelected ? "rgba(212,165,55,0.4)" : "rgba(255,255,255,0.04)"),
          borderRadius: 9,
          marginBottom: 6,
          cursor: "pointer",
      ...
      >
        ...
        <div
          style={{
            fontSize: 12,
            color: isSelected ? "#D4A537" : "rgba(212,165,55,0.7)",
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            style={{ flex: 1, fontSize: 11, color: "rgba(255,255,255,0.6)" }}
          >
