...
export function useMatchmaking({ myId, myNickname, myAvatar, mySeat }) {
  ...
  const joinMatchChannel = useCallback(
    (matchId, opponent, role) => {
      const channel = gameRepository.createNineMatchChannel(matchId);
  ...
  useEffect(() => {
    ...
    const channel = gameRepository.createNineInviteChannel(myId);

    channel
      .on("broadcast", { event: "invite" }, async ({ payload }) => {
        // ★ 이미 게임 중이면 자동 거절
        if (matchRef.current) {
          const senderChannel = gameRepository.createNineInviteChannel(
            payload.from,
          );
          ...
          gameRepository.removeRealtimeChannel(senderChannel);
          return; // 모달 안 띄움
        }
    ...
    return () => {
      gameRepository.removeRealtimeChannel(channel);
      inviteChannelRef.current = null;
    };
  }, [myId, joinMatchChannel]);
  ...
  const sendInvite = useCallback(
    async (targetUser) => {
      ...
      const target = gameRepository.createNineInviteChannel(targetUser.id);
      ...
      gameRepository.removeRealtimeChannel(target);
  ...
  const acceptInvite = useCallback(
    async (invite) => {
      ...
      const senderChannel = gameRepository.createNineInviteChannel(invite.from);
      await senderChannel.subscribe();
      await senderChannel.send({
        type: "broadcast",
        event: "invite-response",
  ...
  const leaveMatch = useCallback(async () => {
    ...
    if (ch) {
      ...
      gameRepository.removeRealtimeChannel(ch);
