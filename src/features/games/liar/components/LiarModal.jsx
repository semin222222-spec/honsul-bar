import { useEffect, useRef, useCallback } from "react";
import { motion as Motion, AnimatePresence } from "framer-motion";
import { X } from "lucide-react";
import { useLiarRoom } from "../hooks/useLiarRoom";
import { useLiarGame } from "../hooks/useLiarGame";
import LiarLobby from "./LiarLobby";
import LiarWaitingRoom from "./LiarWaitingRoom";
import LiarWordReveal from "./LiarWordReveal";
import LiarSpeech from "./LiarSpeech";
import LiarVoting from "./LiarVoting";
import LiarResult from "./LiarResult";

/**
 * LiarModal — 라이어 게임 전체 단계 wrapping 풀스크린 모달
 *
 * room.status: waiting → word_reveal → speech → voting → finished
 * room이 없으면 lobby.
 */
export default function LiarModal({
  open,
  onClose,
  sessionId,
  seatLabel,
  storeId,
}) {
  const room = useLiarRoom({ sessionId, seatLabel, storeId });

  // 결과 자동 만료 → 게임 모달 전체 닫기 (캐치마인드와 동일). DB 정리는 백그라운드.
  const handleAutoDismiss = useCallback(() => {
    onClose();
    room.leaveRoom().catch((err) => {
      console.error("[Liar] 자동 종료 leaveRoom 에러:", err);
    });
  }, [room, onClose]);

  const game = useLiarGame({
    room: room.myRoom,
    sessionId,
    onRoomUpdate: room.setRoomDirect,
    onLeaveAfterFinish: handleAutoDismiss,
  });

  // ESC로 닫기 (lobby에서만)
  useEffect(() => {
    if (!open) return;
    const handler = (e) => {
      if (e.key === "Escape" && !room.myRoom) {
        onClose();
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [open, room.myRoom, onClose]);

  // 모달 unmount 시 자동 leave (유령 방 방지)
  const leaveRoomRef = useRef(room.leaveRoom);
  const myRoomRef = useRef(room.myRoom);
  useEffect(() => {
    leaveRoomRef.current = room.leaveRoom;
    myRoomRef.current = room.myRoom;
  });
  useEffect(() => {
    return () => {
      if (myRoomRef.current) {
        leaveRoomRef.current?.();
      }
    };
  }, []);

  const handleCreate = useCallback(async () => {
    const res = await room.createRoom();
    if (!res.ok) alert(res.error || "방을 만들지 못했어요");
  }, [room]);

  const handleJoin = useCallback(
    async (roomId) => {
      const res = await room.joinRoom(roomId);
      if (!res.ok) alert(res.error || "입장에 실패했어요");
    },
    [room],
  );

  // 나가기 = 게임 모달 전체 닫기 (캐치마인드와 동일). DB 정리는 백그라운드.
  const handleLeaveToLobby = useCallback(() => {
    onClose();
    room.leaveRoom().catch((err) => {
      console.error("[Liar] 백그라운드 leaveRoom 에러:", err);
    });
  }, [room, onClose]);

  const handleStart = useCallback(
    async (selectedCategory) => {
      return await room.startGame(selectedCategory);
    },
    [room],
  );

  const handlePass = useCallback(async () => {
    const res = await game.handlePassMySpeech();
    if (!res?.ok && res?.error) alert(res.error);
  }, [game]);

  if (!open) return null;

  const r = room.myRoom;
  let content = null;

  if (!r) {
    content = (
      <LiarLobby
        rooms={room.rooms}
        loading={room.loading}
        onCreate={handleCreate}
        onJoin={handleJoin}
        onCloseGame={onClose}
        mySeat={seatLabel}
      />
    );
  } else if (r.status === "waiting") {
    content = (
      <LiarWaitingRoom
        room={r}
        isHost={room.isHost}
        sessionId={sessionId}
        onLeave={handleLeaveToLobby}
        onStart={handleStart}
      />
    );
  } else if (r.status === "word_reveal") {
    content = (
      <LiarWordReveal
        room={r}
        sessionId={sessionId}
        onConfirm={game.confirmWord}
      />
    );
  } else if (r.status === "speech") {
    content = (
      <LiarSpeech
        room={r}
        sessionId={sessionId}
        secondsLeft={game.secondsLeft}
        onPass={handlePass}
      />
    );
  } else if (r.status === "voting") {
    content = (
      <LiarVoting
        room={r}
        onReveal={game.revealResult}
        onLeave={handleLeaveToLobby}
      />
    );
  } else if (r.status === "finished") {
    content = (
      <LiarResult
        room={r}
        onLeave={handleLeaveToLobby}
        dismissLeftMs={game.dismissLeftMs}
      />
    );
  }

  return (
    <Motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      style={{
        position: "fixed",
        inset: 0,
        background: "#0F0E0D",
        zIndex: 450,
        overflow: "auto",
        WebkitOverflowScrolling: "touch",
        fontFamily: "'Pretendard Variable', 'Pretendard', system-ui",
      }}
    >
      {!r && (
        <button
          onClick={onClose}
          aria-label="닫기"
          style={{
            position: "fixed",
            top: "max(16px, env(safe-area-inset-top))",
            right: 16,
            zIndex: 5,
            width: 36,
            height: 36,
            borderRadius: "50%",
            background: "rgba(240,232,216,0.08)",
            border: "1px solid rgba(240,232,216,0.15)",
            color: "rgba(240,232,216,0.7)",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            cursor: "pointer",
            WebkitTapHighlightColor: "transparent",
            padding: 0,
          }}
        >
          <X size={18} />
        </button>
      )}
      <AnimatePresence mode="wait">
        <Motion.div
          key={r ? r.status : "lobby"}
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -8 }}
          transition={{ duration: 0.2 }}
          style={{ minHeight: "100vh" }}
        >
          {content}
        </Motion.div>
      </AnimatePresence>
    </Motion.div>
  );
}
