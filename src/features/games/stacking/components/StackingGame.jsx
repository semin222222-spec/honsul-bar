    ...
    >
      <div
        style={{
          ...
          height: "70%",
          background:
            "linear-gradient(180deg, rgba(212,165,55,0.2) 0%, rgba(180,120,40,0.35) 100%)",
      ...
      >
        <div
          style={{
            ...
            height: "60%",
            background:
              "linear-gradient(180deg, rgba(212,165,55,0.5), rgba(160,100,30,0.6))",
...
export default function StackingGame() {
  ...
  }, [phase, dropping, stack, score, maxX]);

  const saveScore = async () => {
    if (!nickname.trim() || saving || !hasStoreScope(storeId)) return;
    setSaving(true);
    ...
    <div style={{ padding: "0 clamp(16px, 4vw, 24px)", paddingTop: 16 }}>
      ...
      {phase === "playing" && (
        <div>
          ...
          >
            ...
            {dropping && !failed && (
              <Motion.div
                ...
                animate={{
                  top:
                    stackBottom - Math.min(stack.length, maxVisible) * GLASS_H,
                }}
                transition={{ type: "spring", stiffness: 350, damping: 18 }}
                style={{ position: "absolute", left: movingX }}
              >
                <WhiskeyGlass />
              </Motion.div>
            )}
          ...
          >
            {locale === "ja" ? "速度" : "속도"}:{" "}
            {"●".repeat(Math.min(Math.round(getSpeed(score)), 8))}
            {"○".repeat(Math.max(0, 8 - Math.round(getSpeed(score))))}
          </div>
        </div>
      )}
            ...
            {!saved ? (
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                <input
                  ...
                  onFocus={(e) =>
                    (e.target.style.borderColor = "rgba(212,165,55,0.3)")
                  }
                  onBlur={(e) =>
