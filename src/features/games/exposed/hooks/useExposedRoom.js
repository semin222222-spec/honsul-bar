 ...
 * useExposedRoom
 *
 * 익명 폭로전 방 lifecycle 관리. (드립/라이어 패턴 그대로)
 *  A) leave 즉시 정리 — leave_exposed_room RPC (0명이면 DB가 DELETE)
 *  B) Heartbeat 30s
 *  C) 좀비 player 자동 정리 90s
 *  D) 페이지 이탈 — fetch keepalive
 *  E) 로비 진입 시 cleanup RPC 1회
 */
export function useExposedRoom({ sessionId, seatLabel, storeId }) {
  ...
  const refreshRooms = useCallback(async () => {
    ...
    try {
      const data = await exposedRepository.listRoomsByStore(storeId);
      setRooms(data);
    } catch (err) {
      console.error("[Exposed] 방 목록 조회 실패:", err);
    }
  }, [storeId]);
  ...
  useEffect(() => {
    ...
    exposedRepository
      ...
      .catch((err) => console.error("[Exposed] cleanup RPC 실패:", err));
  }, [storeId]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = exposedRepository.subscribeToStoreRooms({
      storeId,
      onChange: () => {
        refreshRooms();
      },
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const data = await exposedRepository.createRoom({
        store_id: storeId,
      ...
      console.error("[Exposed] 방 생성 실패:", err);
  ...
  const joinRoom = useCallback(
    async (roomId) => {
      ...
      setLoading(true);
      try {
        const room = await exposedRepository.getRoom(roomId);
        ...
        console.error("[Exposed] 방 입장 실패:", err);
  ...
  const leaveRoom = useCallback(async () => {
    ...
    try {
      await exposedRepository.leaveRoomRpc({ roomId, sessionId });
    } catch (err) {
      console.error("[Exposed] 방 퇴장 RPC 실패:", err);
    }
  }, [myRoom, sessionId]);
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Exposed] 게임 시작 실패:", err);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      exposedRepository
        ...
        .catch((err) => console.error("[Exposed] heartbeat 실패:", err));
    };
    tick();
    ...
    const check = () => {
      ...
      zombies.forEach((z) => {
        exposedRepository
          ...
          .catch((err) => console.error("[Exposed] 좀비 정리 실패:", err));
      });
    };
  ...
  useEffect(() => {
    ...
    const fire = () => {
      exposedRepository.sendLeaveBeacon({
