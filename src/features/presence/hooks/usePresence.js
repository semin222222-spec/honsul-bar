...
export function usePresence(seatLabel, inMatch = false, options = {}) {
  ...
  const [myAvatar, setMyAvatar] = useState(
    () => initialAvatar || randomPick(AVATARS),
  );
  const [myStatus, setMyStatus] = useState("hello");
  const channelRef = useRef(null);
  const presencePayloadRef = useRef(null);
  ...
  }, [myNickname, myNicknameJa, myAvatar, seatLabel, myStatus, inMatch]);
  ...
  useEffect(() => {
    ...
    const channel = presenceRepository.createBarPresenceChannel(myId);
    ...
    channel
      ...
      .subscribe((status) => {
        handleRealtimeSubscribeStatus(status, {
          label: "Presence",
          onSubscribed: () => {
            if (presencePayloadRef.current) {
              return channel.track(presencePayloadRef.current);
            }
            return null;
          },
        });
    ...
    return () => {
      if (channelRef.current) {
        ...
        presenceRepository.removePresenceChannel(channelRef.current);
  ...
  useEffect(() => {
    if (channelRef.current && seatLabel && presencePayloadRef.current) {
      channelRef.current.track(presencePayloadRef.current);
    }
  }, [myStatus, myNickname, myNicknameJa, myAvatar, inMatch, seatLabel]);
