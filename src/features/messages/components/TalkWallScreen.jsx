...
export default function TalkWallScreen({ onQuestComplete }) {
  ...
  const { messages, loading, error, postMessage, addHeart, addCurious } =
    useMessages(storeId);
  ...
  return (
    <div style={{ padding: "0 clamp(16px, 4vw, 24px)", paddingTop: 16 }}>
      ...
      >
        ...
        >
          닉네임:
          <span style={{ color: "rgba(212,165,55,0.7)", fontWeight: 500 }}>
            {nickname}
          </span>
        </div>
      </div>
        ...
        >
          ...
          <Motion.button
            ...
            style={{
              ...
              color:
                input.trim() && !posting ? "#0D0B08" : "rgba(255,255,255,0.2)",
        ...
        <div ref={feedRef}>
          {messages.length === 0 ? (
            <Motion.div
              ...
              style={{
                textAlign: "center",
                padding: "clamp(30px, 10vw, 48px) 20px",
                color: "rgba(255,255,255,0.2)",
              }}
            >
              <div
                style={{ fontSize: "clamp(28px, 8vw, 36px)", marginBottom: 12 }}
              >
                🌙
              </div>
              <div
