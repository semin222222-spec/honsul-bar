...
import { FINAL_RESULTS, ROUND_RESULTS } from "../lib/flirtingQuestions";
...
export default function FlirtingGameModal({
  ...
  // 양쪽 다 선택했는지
  const bothChose = !!myChoices[currentRound] && !!opponentChoices[currentRound];
  ...
  // 상대방 정보
  const opponentSeat = isInviter ? game.invitee_seat_label : game.inviter_seat_label;
  const opponentNickname = isInviter ? game.invitee_nickname : game.inviter_nickname;
    ...
    >
      <motion.div
        ...
        style={{
          ...
          background: "linear-gradient(135deg, rgba(40,20,40,0.98), rgba(20,15,30,0.98))",
      ...
      >
        ...
        {(showFinalResult || status === "declined" || status === "cancelled") && (
        ...
        <AnimatePresence mode="wait">
          ...
          {status === "playing" && !showFinalResult && showRoundResult && bothChose && (
            <RoundResultScreen
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        style={{
          padding: "12px 32px",
          background: "linear-gradient(135deg, rgba(255,107,157,0.4), rgba(196,122,255,0.3))",
          border: "1px solid rgba(255,107,157,0.4)",
          borderRadius: 10,
          color: "#fff",
...
// 3. 라운드 진행 화면
// ────────────────────────────────────────
function RoundScreen({ question, round, myChoice, opponentChose, myChoices, opponentChoices, mySeatLabel, opponentSeat, onSelect }) {
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11 }}>
          ...
          <span style={{ color: "rgba(255,200,220,0.7)" }}>
            {mySeatLabel}
          </span>
          <span style={{ color: "#FF6B9D", fontWeight: 700, fontSize: 10 }}>VS</span>
          <span style={{ color: "rgba(255,200,220,0.7)" }}>
            {opponentSeat}
          </span>
          <div style={{
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: isCurrent ? "0 0 8px rgba(255,107,157,0.7)" : "none",
                }}
              />
            );
          })}
      ...
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <div style={{
          ...
          {question.level === "spicy3" ? "🔥 ROUND 5 / 5" :
           question.level === "spicy2" ? "💋 ROUND 4 / 5" :
           question.level === "spicy1" ? "🍒 ROUND 3 / 5" :
           `ROUND ${round} / 5`}
        </div>
        <div style={{
          fontSize: 18,
      ...
      <div style={{
        ...
        {!myChoice && opponentChose && "💌 상대방이 먼저 골랐어요! 어떤 단어를 고를래요?"}
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        border: "2px solid " + (isSelected ? "#FF6B9D" : "rgba(255,255,255,0.08)"),
        borderRadius: 14,
...
// 4. 라운드 결과 화면
// ────────────────────────────────────────
function RoundResultScreen({ question, myChoice, opponentChoice, round, isLastRound, onNext }) {
  ...
  const resultData = isMatch ? ROUND_RESULTS.match[level] : ROUND_RESULTS.mismatch[level];
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: "2px solid " + (isMatch ? "rgba(255,107,157,0.5)" : "rgba(100,150,200,0.4)"),
        textAlign: "center",
      }}
    >
      <motion.div
    ...
    >
      ...
      >
        <div style={{
          ...
          {score}<span style={{ fontSize: 30, opacity: 0.6 }}>/5</span>
        </div>
        <div style={{
          fontSize: 10,
        ...
        <div style={{
          ...
          상대방: <strong style={{ color: result.color }}>📍 {opponentSeat}</strong>
          {opponentNickname && (
